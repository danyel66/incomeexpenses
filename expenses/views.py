...
def add_expense(request):
    ...
    if request.method == 'POST':
        ...
        if not category:
            messages.error(request, 'please select a category from the dropdown menu')
            return render(request, 'expenses/add_expenses.html', context)
...
def export_excel(request):
    ...
    columns = ['Amount', 'Descriotion', 'Category', 'Date']
...
def export_pdf(request):
    ...
    html_string = render_to_string(
        'expenses/pdf.html', {'expenses': expenses , 'total': sum['amount__sum']}
    )
